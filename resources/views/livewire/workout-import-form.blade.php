    ...
    <div class="mx-auto flex max-w-4xl flex-col gap-10 px-4 pb-16 pt-10 sm:px-6 lg:px-8">
        <header class="flex flex-col gap-6">
            ...
            <div class="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 bg-gradient-to-br from-white/10 to-white/5 p-6 backdrop-blur">
                @if ($job)
                    <div wire:poll.3s="refreshState" class="flex flex-col gap-4" aria-live="polite">
                        <div class="flex items-center gap-4">
                            <span class="flex h-16 w-16 flex-none items-center justify-center rounded-2xl bg-white/5">
                                <svg class="h-10 w-10 animate-spin text-lime-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z"></path>
                                </svg>
                            </span>
                            <div class="flex flex-col gap-1">
                                <h2 class="text-lg font-semibold text-white">{{ __('Przetwarzanie importu…') }}</h2>
                                <p class="text-sm text-slate-400">{{ __('Może to potrwać kilka minut. Możesz pozostać na tej stronie.') }}</p>
                            </div>
                        </div>
                        <div class="h-2 w-full overflow-hidden rounded-full bg-white/10">
                            <div class="h-full w-1/2 animate-pulse bg-lime-300/70"></div>
                        </div>
                    </div>
                @else
