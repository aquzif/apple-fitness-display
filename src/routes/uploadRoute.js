...
async function handleUpload(req, res) {
  ...
  try {
    const xmlPayload = await fsp.readFile(filePath, 'utf8');
    const parsedDocument = parser.parseFromString(xmlPayload);
    const rawWorkouts = parser.extractWorkoutNodes(parsedDocument);
